...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    const ourProgrammer = programmers.filter(dev => dev.id===activeProgrammer)[0];
    if (!ourProgrammer) return undefined;
    if (ourProgrammer) console.log(ourProgrammer);
    return ourProgrammer.name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: activeProgrammer?'gold':'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { {/* in here set the featured id to be dev.id */} setActiveProgrammer(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
