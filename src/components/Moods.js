...
export default function Moods() {
  ...
  return (
    <div className='widget-moods container'>
      ...
      <div>
        <button id='makeHappy' onClick={makeHappy}>{happyMood}</button>
        <button id='makeSad' onClick={makeSad}>{sadMood}</button>
        <button id='resetMood' onClick={reset}>{initialMood}</button>
      </div>
    </div>
  );
}
